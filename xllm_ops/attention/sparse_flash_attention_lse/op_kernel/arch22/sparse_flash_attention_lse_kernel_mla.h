...
/*!
 * \file sparse_flash_attention_lse_kernel_mla.h
 * \brief
 */

#ifndef SPARSE_FLASH_ATTENTION_LSE_KERNEL_MLA_H
#define SPARSE_FLASH_ATTENTION_LSE_KERNEL_MLA_H
...
#include "../sparse_flash_attention_lse_common.h"
#include "sparse_flash_attention_lse_service_cube_mla.h"
#include "sparse_flash_attention_lse_service_vector_mla.h"
...
template <typename SFAT> class SparseFlashAttentionLseMla {
public:
    ...
    __aicore__ inline SparseFlashAttentionLseMla(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                                ...
                                __gm__ uint8_t *workspace, const SparseFlashAttentionLseTilingDataMla *__restrict tiling,
...
private:
    ...
    static constexpr uint64_t headDimRope = SFAT::hasRope ? 64ULL : 0ULL;
    static constexpr uint32_t msdIterNum = 2U;

    static constexpr uint32_t dbWorkspaceRatio = PRELOAD_NUM;

    const SparseFlashAttentionLseTilingDataMla *__restrict tilingData = nullptr;
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionLseMla<SFAT>::InitTilingData()
{
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionLseMla<SFAT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
SparseFlashAttentionLseMla<SFAT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::InitAllZeroOutput(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::InitOutputSingleCore()
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::GetActualSeqLen(uint32_t bIdx, uint32_t s1Idx)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::GetSparseActualSeqLen(uint32_t bIdx, uint32_t s1Idx,
                                                                            uint32_t n2Idx)
{
...
template <typename SFAT>
__aicore__ inline uint32_t SparseFlashAttentionLseMla<SFAT>::GetActualSeqLenKV(uint32_t bIdx)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
    if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::GetPreNextTokensLeftUp()
{
    if (constInfo.sparseMode == 3) {
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionLseMla<SFAT>::UpdateInnerLoopCond()
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::UpdateInner(uint32_t &s2End, uint32_t &curS2End,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::Init(__gm__ uint8_t *query,
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionLseMla<SFAT>::InitCalcParamsEach()
{
    //计算总的基本块
...
template <typename SFAT>
__aicore__ inline void
SparseFlashAttentionLseMla<SFAT>::Bmm2DataCopyOut(uint64_t attenOutOffset, LocalTensor<OUT_T> &attenOutUb,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::CalcParams(uint32_t loop, uint64_t s2Start,
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::ComputeMm1(const RunInfo &info)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::ComputeMm2(const RunInfo &info)
{
    uint32_t nBufferLoopTimes = (info.actMBaseSize + constInfo.nBufferMBaseSize - 1) / constInfo.nBufferMBaseSize;
    uint32_t nBufferTail = info.actMBaseSize - (nBufferLoopTimes - 1) * constInfo.nBufferMBaseSize;
    for (uint32_t i = 0; i < nBufferLoopTimes; i++) {
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionLseMla<SFAT>::Process()
{
    if (aiCoreIdx < usedCoreNum) {
        if ASCEND_IS_AIV {
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::GetBN2Idx(uint32_t bN2Idx, uint32_t &bIdx,
                                                                                uint32_t &n2Idx)
{
...
template <typename SFAT> __aicore__ inline void SparseFlashAttentionLseMla<SFAT>::ProcessBalance()
{
...
template <typename SFAT>
__aicore__ inline void
SparseFlashAttentionLseMla<SFAT>::PreloadPipeline(uint32_t loop, uint64_t s2Start, uint64_t s2LoopIdx,
                                                               RunInfo extraInfo[SFA_PRELOAD_TASK_CACHE_SIZE], uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
{
...
SparseFlashAttentionLseMla<SFAT>::GetBalanceActualSeqLengths(GlobalTensor<int32_t> &actualSeqLengths,
                                                                          uint32_t bIdx)
{
    if constexpr (LAYOUT_T == SFA_LAYOUT::TND) {
        if (bIdx > 0) {
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::GetAxisStartIdx(uint32_t bN2EndPrev,
                                                                                      uint32_t s1GEndPrev,
                                                                                      uint32_t s2EndPrev)
{
...
template <typename SFAT>
__aicore__ inline void SparseFlashAttentionLseMla<SFAT>::CalcSinnerTopKBegin(RunInfo &info, uint32_t &curTopKIdx, uint64_t &curOffsetInSparseBlock)
...
#endif // SPARSE_FLASH_ATTENTION_LSE_KERNEL_MLA_H
